...
def fill_full_training_data(full_training_data, start_date, end_date, stocks, api_key):
    ...
    # for each stock  query triing API for raw sata and then populate the full_training_data dataframe
    for stock in stocks:
        stock_data = get_tiingo_daily_stock_data(stock, api_key, start_date, end_date)
        # for each date in the stock_data, populate the full_training_data dataframe with % change of adjOpen and adjClose
        for date in stock_data.index:
            full_training_data.loc[date, stock] = (stock_data.loc[date, 'adjClose'] - stock_data.loc[date, 'adjOpen']) / stock_data.loc[date, 'adjOpen'] * 100
